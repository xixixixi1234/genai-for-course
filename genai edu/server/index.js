...
// Add prompt_cues column to cases (JSON array, NULL = use global cues)
try {
  db.exec(`ALTER TABLE cases ADD COLUMN prompt_cues TEXT`);
} catch (e) { /* column already exists */ }

// Add roles column to cases (JSON array of {id, label, brief}; NULL = no roles)
try {
  db.exec(`ALTER TABLE cases ADD COLUMN roles TEXT`);
} catch (e) { /* column already exists */ }

// Add role_id column to messages (tracks which role the student was when they sent it)
try {
...
app.get('/api/cases/:id', authStudent, (req, res) => {
  ...
  // Parse per-case prompt_cues (JSON column) so the client gets an array, not a string
  if (c.prompt_cues) {
    try { c.prompt_cues = JSON.parse(c.prompt_cues); }
    catch (e) { c.prompt_cues = null; }
  }
  if (c.roles) {
    try { c.roles = JSON.parse(c.roles); }
    catch (e) { c.roles = null; }
  }
  res.json({ case: c });
});
...
app.get('/api/messages', authStudent, (req, res) => {
  ...
  if (caseId) {
    rows = db.prepare(
      'SELECT id, role, content, role_id, created_at FROM messages WHERE student_id = ? AND case_id = ? ORDER BY created_at ASC'
    ).all(req.student.id, caseId);
  } else {
    rows = db.prepare(
      'SELECT id, role, content, role_id, created_at FROM messages WHERE student_id = ? AND case_id IS NULL ORDER BY created_at ASC'
    ).all(req.student.id);
  }
...
app.post('/api/chat', authStudent, async (req, res) => {
  ...
  const { message, case_id, stage_id, role_id } = req.body;
  ...
  // Validate case_id and load roles
  let validCaseId = null;
  let currentCase = null;
  let currentRole = null;
  if (case_id) {
    currentCase = db.prepare('SELECT title, description, content, roles FROM cases WHERE id = ?').get(case_id);
    if (!currentCase) {
      return res.status(400).json({ error: 'Case not found' });
    }
    validCaseId = case_id;
    // Parse roles and find current
    let parsedRoles = null;
    if (currentCase.roles) {
      try { parsedRoles = JSON.parse(currentCase.roles); } catch (e) {}
    }
    if (role_id && Array.isArray(parsedRoles)) {
      currentRole = parsedRoles.find(r => r.id === role_id) || null;
    }
  }

  const startTs = now();
  const validRoleId = currentRole ? currentRole.id : null;

  // Save user message (with role_id)
  const userMsgResult = db.prepare(
    'INSERT INTO messages (student_id, case_id, role, content, role_id, created_at) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(req.student.id, validCaseId, 'user', message, validRoleId, startTs);
  ...
  // Role-specific context (if student selected a role for this case)
  if (currentRole) {
    systemPrompt += `\n\n--- THE STUDENT'S ROLE: ${currentRole.label.toUpperCase()} ---\nFor this conversation, the student is playing the role of ${currentRole.label}.${currentRole.brief ? `\n\nWhat this role knows / cares about:\n${currentRole.brief}` : ''}\n\nFrame your guidance from this role's perspective. Help them think about the situation from where ${currentRole.label} sits — their goals, constraints, and what they personally would and would not know. Don't break character to share info this role wouldn't have. If they ask something outside their role's scope, gently redirect them back to their position.`;
  }
  ...
  try {
    ...
    // Save assistant message (with role_id matching the conversation context)
    db.prepare(
      'INSERT INTO messages (student_id, case_id, role, content, role_id, response_ms, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)'
    ).run(req.student.id, validCaseId, 'assistant', reply, validRoleId, elapsed, now());
...
// Helper: clean and validate a roles array — used by POST and PATCH
function cleanRoles(input) {
  ...
  const cleaned = input
    ...
    .slice(0, 12)
    .map(r => ({
      id: (r.id || r.label).toString().toLowerCase().replace(/[^a-z0-9_-]+/g, '_').slice(0, 40) || 'role',
      label: r.label.trim().slice(0, 60),
      brief: (r.brief || '').toString().trim().slice(0, 2000)
    }));
  return cleaned;
}
...
app.post('/api/admin/cases', authAdmin, upload.single('file'), (req, res) => {
  ...
  // Optional per-case cues (JSON-encoded string from form field)
  let cuesJson = null;
  if (req.body.prompt_cues) {
    try {
      const parsed = JSON.parse(req.body.prompt_cues);
      if (Array.isArray(parsed)) {
        ...
        cuesJson = JSON.stringify(cleaned);
      }
    } catch (e) { /* ignore invalid JSON */ }
  }

  // Optional per-case roles (JSON-encoded string from form field)
  let rolesJson = null;
  if (req.body.roles) {
    try {
      const parsed = JSON.parse(req.body.roles);
      const cleaned = cleanRoles(parsed);
      if (cleaned && cleaned.length > 0) rolesJson = JSON.stringify(cleaned);
    } catch (e) { /* ignore invalid JSON */ }
  }

  const filePath = req.file ? '/uploads/' + req.file.filename : null;
  const result = db.prepare(
    'INSERT INTO cases (title, description, content, file_path, prompt_cues, roles, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)'
  ).run(title, description, content, filePath, cuesJson, rolesJson, now());

  res.json({ id: result.lastInsertRowid });
});

// Update case (admin) — supports editing prompt_cues and/or roles
app.patch('/api/admin/cases/:id', authAdmin, (req, res) => {
  const c = db.prepare('SELECT * FROM cases WHERE id = ?').get(req.params.id);
  if (!c) return res.status(404).json({ error: 'Case not found' });

  if (req.body.prompt_cues !== undefined) {
    if (req.body.prompt_cues === null || req.body.prompt_cues === '') {
      db.prepare('UPDATE cases SET prompt_cues = NULL WHERE id = ?').run(req.params.id);
    } else if (Array.isArray(req.body.prompt_cues)) {
      const cleaned = req.body.prompt_cues
        .filter(x => x && typeof x.label === 'string' && typeof x.text === 'string' && x.label.trim() && x.text.trim())
        .slice(0, 20)
        .map(x => ({ label: x.label.trim().slice(0, 80), text: x.text.trim().slice(0, 600) }));
      db.prepare('UPDATE cases SET prompt_cues = ? WHERE id = ?').run(JSON.stringify(cleaned), req.params.id);
    }
  }
  if (req.body.roles !== undefined) {
    if (req.body.roles === null) {
      db.prepare('UPDATE cases SET roles = NULL WHERE id = ?').run(req.params.id);
    } else {
      const cleaned = cleanRoles(req.body.roles);
      if (cleaned && cleaned.length > 0) {
        db.prepare('UPDATE cases SET roles = ? WHERE id = ?').run(JSON.stringify(cleaned), req.params.id);
      } else {
        db.prepare('UPDATE cases SET roles = NULL WHERE id = ?').run(req.params.id);
      }
    }
...
// ───── Danger zone: wipe all conversation data ─────
// Wipes all messages and sessions. Keeps students, cases, settings.
// Optional: ?wipe_students=1 to also remove student accounts (forces all to log in again).
app.delete('/api/admin/wipe-conversations', authAdmin, (req, res) => {
  try {
    const counts = {
      messages_before: db.prepare('SELECT COUNT(*) as c FROM messages').get().c,
      sessions_before: db.prepare('SELECT COUNT(*) as c FROM sessions').get().c,
    };
    db.prepare('DELETE FROM messages').run();
    db.prepare('DELETE FROM sessions').run();
    if (req.query.wipe_students === '1') {
      counts.students_before = db.prepare('SELECT COUNT(*) as c FROM students').get().c;
